...
interface AusbaugebietDashboardProps {
  ...
  onCreateAusbaugebiet: (data: Partial<Ausbaugebiet>) => Promise<void>;
  onUpdateAusbaugebiet: (ausbaugebiet: Ausbaugebiet) => Promise<void>;
  onDeleteAusbaugebiet: (ausbaugebietId: string) => Promise<void>;
  onOpenSettings: () => void;
  accentColor: string;
}
...
export function AusbaugebietDashboard({
  ...
  onDeleteAusbaugebiet,
  onOpenSettings,
  accentColor,
}: AusbaugebietDashboardProps) {
  const [showModal, setShowModal] = useState(false);
  const [editingAusbaugebiet, setEditingAusbaugebiet] = useState<Ausbaugebiet | null>(null);
  ...
  const openEditModal = (proj: Ausbaugebiet, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingAusbaugebiet(proj);
    setFormData({
      name: proj.name,
  ...
  const handleDelete = async (proj: Ausbaugebiet, e: React.MouseEvent) => {
    e.stopPropagation();
    if (ausbaugebiets.length <= 1) {
    ...
    if (ok) {
      await onDeleteAusbaugebiet(proj.id);
    }
  };
        ...
        {ausbaugebiets.map(proj => {
          ...
          return (
            <div
              ...
              onClick={() => onSelectAusbaugebiet(proj.id)}
            >
        ...
        <div style={styles.modalOverlay} onClick={() => setShowModal(false)}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
              <div>
                <label style={styles.formLabel}>Name des Ausbaugebiets *</label>
                <input
                  type="text"
                  required
                  placeholder="z. B. Rauenzell"
                  value={formData.name}
