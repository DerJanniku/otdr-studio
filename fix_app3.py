...
# Assemble everything
# Wait, I need to extract the parts properly because `c` has been changed by `re.sub`.
# Let's do `re.sub` AFTER splicing.

new_c = orig.replace("import type { CustomerItem, AppSettings, Project } from './types';", "import type { CustomerItem, AppSettings, Project, Ausbaugebiet, KVZ } from './types';")
new_c = new_c.replace("import { ProjectDashboard } from './components/ProjectDashboard';", "import { ProjectDashboard } from './components/ProjectDashboard';\nimport { AusbaugebietDashboard } from './components/AusbaugebietDashboard';\nimport { KvzDashboard } from './components/KvzDashboard';")

new_c = new_c.replace("const [view, setView] = useState<'dashboard' | 'project'>('dashboard');", 
"""const [view, setView] = useState<'dashboard' | 'ausbaugebiet' | 'kvz' | 'customer'>('dashboard');
  const [ausbaugebiete, setAusbaugebiete] = useState<Ausbaugebiet[]>([]);
  const [activeAusbaugebiet, setActiveAusbaugebiet] = useState<Ausbaugebiet | null>(null);
  const [kvzs, setKvzs] = useState<KVZ[]>([]);
...
new_c = new_c.replace("const loadData = async () => {", loaders + "\n  const loadData = async () => {")
new_c = new_c.replace("""    if (window.api?.getCustomers) {
      const list = await window.api.getCustomers();
      setCustomers(list);
    }""", "")

start_idx = new_c.find("{view === 'dashboard' ? (")
end_idx = new_c.find("{/* PREVIEW & EDIT MODAL */}")
mid_idx = new_c.find(") : (", start_idx, end_idx)

cust_view_code = new_c[mid_idx+5:end_idx].strip()
if cust_view_code.endswith(")}"):
    cust_view_code = cust_view_code[:-2]
...
new_c = new_c[:start_idx] + view_render + "\n<>\n" + cust_view_code + "\n</>\n      )}\n\n      " + new_c[end_idx:]

new_c = re.sub(r'const handleImportExcel = async \(\) => \{.*?(?=\s*const handleSaveSettings)', import_logic, new_c, flags=re.DOTALL)
