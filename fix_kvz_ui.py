...
# Add states for measurements to formData
c = c.replace("name: '',", "name: '',\n    popMeasurements: [] as PopMeasurement[],")
c = c.replace("name: proj.name,", "name: proj.name,\n      popMeasurements: proj.popMeasurements || [],")

# Import PopMeasurement type
c = c.replace("import type { KVZ } from '../types';", "import type { KVZ, PopMeasurement } from '../types';")

# In the modal, below 'Beschreibung' (which was ClusterName), add the Zuleitungsmessungen UI
ui_add = """            </div>
                ...
                <div key={pm.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', background: 'rgba(255,255,255,0.05)', padding: '0.5rem', borderRadius: '6px' }}>
                  ...
                  <button type="button" onClick={async () => {
                     if (window.api?.chooseDirectory) {
                        ...
                        // Wait, we can't use electron dialog in renderer!
                        // We must use a new IPC method: window.api.selectSorFile()
...
"""
c = re.sub(r'            </div>\s*<div style=\{\{ display: \'flex\', justifyContent: \'flex-end\', gap: \'1rem\' \}\}>', ui_add + r'\n            <div style={{ display: \'flex\', justifyContent: \'flex-end\', gap: \'1rem\' }}>', c, flags=re.DOTALL)
